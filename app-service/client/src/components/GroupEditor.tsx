  ...
  const [formData, setFormData, isDirty] = useDirtyState({
    ...
    members:
      group?.members?.map((m: unknown) =>
        typeof m === 'object' && m !== null && 'id' in m ? (m as { id: string }).id : m,
      ) || [],
      ...
      <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex sm:items-center sm:justify-center p-0 sm:p-4">
        <div className="bg-matrix-light sm:rounded-3xl p-6 w-full max-w-2xl flex flex-col h-full sm:h-auto sm:max-h-[90vh] overflow-hidden">
          <div className="flex justify-between items-center mb-6 shrink-0">
            <h2 data-testid="group-editor-title" className="text-2xl font-bold text-white">
              {group?.displayName || group?.name}
            </h2>
            <button onClick={onCancel} className="p-2 hover:bg-white/5 rounded-full text-matrix-muted">
              <X />
            </button>
          </div>
          <div className="space-y-4 overflow-y-auto flex-1 custom-scrollbar pb-4">
            ...
            <div className="flex flex-wrap gap-2">
              {systemMembers
                .filter((m) => (formData.members as string[]).includes(m.id))
                .map((m) => (
                  <span
                    key={m.id}
                    className="bg-matrix-dark px-3 py-1 rounded-full text-sm font-medium border border-white/5"
                  >
                    {m.name}
                  </span>
                ))}
              {formData.members.length === 0 && (
                <span className="text-matrix-muted italic">No members in this group.</span>
              )}
            </div>
          </div>
        </div>
    ...
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex sm:items-center sm:justify-center p-0 sm:p-4">
      <div className="bg-matrix-light sm:rounded-3xl p-6 w-full max-w-2xl sm:border border-white/10 shadow-2xl relative flex flex-col h-full sm:h-auto sm:max-h-[90vh] overflow-hidden">
        <div className="flex justify-between items-start sm:items-center mb-6 shrink-0">
          ...
          <button onClick={handleCancel} className="p-2 hover:bg-white/5 rounded-full text-matrix-muted">
            <X />
          </button>
        </div>
                ...
                <div className="flex flex-col md:flex-row gap-8 items-start">
                  ...
                  <div className="flex-1 space-y-4 w-full">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div>
                        <label className="block text-xs font-bold text-matrix-muted uppercase tracking-wider mb-1">
                    ...
                    <div className="grid grid-cols-1 gap-4">
                      <div>
                        <label className="block text-xs font-bold text-matrix-muted uppercase tracking-wider mb-1">
                          Slug (ID) <span className="font-normal opacity-50 ml-1">Optional</span>
                        </label>
                        <input
                          name="group-slug"
                          className="matrix-input w-full"
                    ...
                    <div className="flex flex-wrap gap-2 bg-matrix-dark/30 p-4 rounded-xl border border-white/5">
                      {systemMembers
                        ...
                        .map((member) => {
                          ...
                          const testId = member.name
                            ? `toggle-member-${member.name.toLowerCase().replace(/[^a-z0-9]/g, '-')}`
                            : '';
                          return (
                            <button
                              key={member.id}
                  ...
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-xs text-matrix-muted">
                        If private, this group will not appear in group lists for others.
                      </p>
                    </div>
                    <PrivacyToggle
          ...
          <div className="pt-4 flex justify-end space-x-3 border-t border-white/5 shrink-0">
            ...
            >
              {saving ? (
                ...
                <>
                  <Save size={18} className="mr-2" /> Save Group
                </>
              )}
            </button>
          </div>
