...
const DashboardPage: React.FC = () => {
  ...
  const fetchData = React.useCallback(
    async (isBackground = false) => {
      ...
      try {
        ...
        if ((err as { response?: { status?: number } }).response?.status === 404 && token && systemRef.current) {
  ...
  const handleDeleteSystem = async () => {
    ...
    if (
      confirm(
        '⚠️ WARNING: This will permanently delete your ENTIRE SYSTEM, including all members, and disconnect your accounts. This cannot be undone. Are you absolutely sure?',
      )
    ) {
      try {
      ...
      <div className="min-h-screen flex flex-col items-center justify-center bg-matrix-dark text-matrix-text px-4">
        <div className="bg-matrix-light p-8 rounded-3xl border border-white/5 text-center space-y-6 max-w-md shadow-2xl">
          ...
          <p className="text-matrix-muted">
            The system with slug <b>{urlSlug}</b> could not be found or is not public.
          </p>
    ...
    <div className="min-h-screen pb-20 text-matrix-text">
        ...
        <div className="max-w-7xl mx-auto px-4 h-20 flex items-center justify-between">
          <div className="flex items-center space-x-4 overflow-hidden">
            <Link to="/">
              <img
                src="/lily.png"
                alt="PluralMatrix Logo"
                className="w-10 h-10 rounded-xl object-cover shadow-lg hover:scale-105 transition-transform"
              />
            </Link>
            ...
            {token ? (
              <>
                {!isOwner && (
                  <Link
                    to="/"
                    className="text-sm font-bold text-matrix-primary hover:text-matrix-primary/80 transition-colors hidden md:block"
                  >
                    My Dashboard
                  </Link>
                )}
          ...
          <div className="flex items-center gap-6">
            ...
            <div className="space-y-2">
              <div className="space-y-1">
                ...
                {Boolean(system?.systemTag) && (
                  <div className="text-xl font-normal text-matrix-muted/80 flex items-center">
                    <span className="bg-white/5 px-2 py-0.5 rounded text-sm uppercase tracking-wider mr-2 text-xs font-bold font-mono">
                      Suffix Tag
                    </span>
              ...
              {Boolean(system?.description) && (
                <p className="text-matrix-muted/80 text-sm italic max-w-2xl mt-2 line-clamp-3">
          ...
          {isOwner && (
            <div className="flex items-center gap-3">
              ...
              <div className="relative">
                ...
                >
                  <Database size={18} className="mr-2" /> Data{' '}
                  <ChevronDown
                    size={16}
                    className={`ml-2 transition-transform ${isDataMenuOpen ? 'rotate-180' : ''}`}
                  />
                </button>
                ...
                <AnimatePresence>
                  {isDataMenuOpen && (
                    <>
                      ...
                      >
                        <div className="px-4 py-2 text-[10px] font-bold text-matrix-muted uppercase tracking-wider">
                          Export
                        </div>
                        <button
                          onClick={() => {
                            memberService.exportBackupZip();
                        ...
                        >
                          <Archive
                            size={16}
                            className="mr-3 text-matrix-primary group-hover:scale-110 transition-transform"
                          />
                          <div className="font-bold">Export Backup (ZIP)</div>
                        </button>
                        <button
                          onClick={() => {
                            memberService.exportPkZip();
                            setIsDataMenuOpen(false);
                          }}
                        ...
                        <div className="h-px bg-white/5 my-2" />
                        <div className="px-4 py-2 text-[10px] font-bold text-red-400 uppercase tracking-wider">
                          Danger Zone
                        </div>
                        <button
                          onClick={() => {
        ...
        <div className="flex flex-col md:flex-row gap-4 items-center justify-between bg-matrix-light p-4 rounded-2xl border border-white/5 shadow-inner">
          ...
          <div className="flex flex-col md:flex-row gap-4 items-center w-full md:w-auto flex-1 md:justify-end">
            ...
            <div className="flex items-center bg-matrix-dark/50 p-1 rounded-lg hidden md:flex">
              <button className="p-2 bg-matrix-light shadow-sm rounded-md text-matrix-primary">
                <LayoutGrid size={18} />
              </button>
              <button className="p-2 text-matrix-muted hover:text-white transition-colors">
                <List size={18} />
              </button>
            </div>
          </div>
        ...
        {((activeTab === 'members' && filteredMembers.length === 0) ||
          ...
          <div className="text-center py-20 space-y-4">
            ...
            <p className="text-matrix-muted max-w-xs mx-auto">
              {isOwner
                ? `Try a different search term or add your first ${activeTab === 'members' ? 'member' : 'group'} using the button above.`
      ...
      {isEditing && (
        <MemberEditor
          ...
          systemGroups={groups as React.ComponentProps<typeof MemberEditor>['systemGroups']}
          isReadOnly={!isOwner}
          onSave={() => {
            setIsEditing(false);
            fetchData(true);
      ...
      {isEditingGroup && (
        <GroupEditor
          ...
          systemMembers={members as React.ComponentProps<typeof GroupEditor>['systemMembers']}
          isReadOnly={!isOwner}
          onSave={() => {
