...
describe('API Endpoints', () => {
  ...
  describe('POST /api/auth/login', () => {
    it('should return 200 and a token on valid login', async () => {
      ...
      const response = await request(app).post('/api/auth/login').send({ mxid: mockMxid, password: 'password' });
  ...
  describe('Member CRUD API', () => {
    ...
    it('GET /api/members should return member list', async () => {
      ...
      const response = await request(app).get('/api/members').set(authHeader);
    ...
    it('PATCH /api/members/:id should update existing member', async () => {
      ...
      const response = await request(app).patch('/api/members/m1').set(authHeader).send({ name: 'Lily Updated' });
    ...
    it('DELETE /api/members/:id should remove member', async () => {
      ...
      const response = await request(app).delete('/api/members/m1').set(authHeader);
    ...
    it('DELETE /api/members (Bulk) should remove all members', async () => {
      ...
      const response = await request(app).delete('/api/members').set(authHeader);
    ...
    it('POST /api/members should FAIL if required fields are missing', async () => {
      ...
      const response = await request(app).post('/api/members').set(authHeader).send({
    ...
    it('POST /api/members should FAIL if duplicate proxy tags in system', async () => {
      (prisma.accountLink.findUnique as jest.Mock).mockResolvedValue({
        system: {
          id: 'sys1',
          members: [{ name: 'Lily', proxyTags: [{ prefix: 'l:', suffix: '' }] }],
        },
      });
  ...
  describe('System Settings API', () => {
    ...
    it('GET /api/system should return system details', async () => {
      ...
      const response = await request(app).get('/api/system').set(authHeader);
    ...
    it('PATCH /api/system should update system details', async () => {
      ...
      const response = await request(app).patch('/api/system').set(authHeader).send({ name: 'New Name' });
