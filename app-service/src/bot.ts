...
export const initCommandHandler = (
  ...
  cryptoManagerInstance: OlmMachineManager,
  token: string,
  domainStr: string,
) => {
...
export const handleEvent = async (
          ...
          if (system) {
            ...
            if (primaryLink) {
              ...
              try {
                ...
                const senderProfile = (await intentClient.matrixClient.getUserProfile(sender)) as {
                  displayname?: string;
                };
                const ghostProfile = (await intentClient.matrixClient.getUserProfile(targetUserId)) as {
                  displayname?: string;
                };
              ...
              if (!isOwnerInviting) {
                ...
                console.log(
                  `[Ghost] Owner ${maskMxid(sender)} invited managed ghost. Skipping owner invite, inviting bot.`,
                );
              }
    ...
    if (membership === 'join') {
      try {
        ...
        if (ghostInRoom) {
          ...
          if (system) {
            const primaryUser =
              system.accountLinks.find((l) => l.isPrimary)?.matrixId || system.accountLinks[0].matrixId;
  ...
  if (event.type === 'm.reaction') {
    ...
    if (relatesTo?.rel_type === 'm.annotation') {
      ...
      if (reaction?.includes('❌') || reaction?.toLowerCase() === 'x' || reaction?.toLowerCase() === ':x:') {
        ...
        try {
          const targetEvent = (await bridgeInstance.getBot().getClient().getEvent(roomId, targetEventId)) as {
            sender: string;
          };
          if (targetEvent && targetEvent.sender.startsWith(`@_plural_${system.slug}_`)) {
            ...
            await commandHandler.safeRedact(
              roomId,
              targetEventId,
              'UserRequest',
              bridgeInstance.getIntent(targetEvent.sender),
            );
  ...
  if (isEdit) {
    try {
      ...
      originalEvent = await fetchAndDecryptHistoricalEvent(
        botClient,
        roomId,
        originalEventId,
        cryptoManager,
  ...
  if (proxyMatch) {
    ...
    try {
      ...
      const finalDisplayName = system.systemTag
        ? `${targetMember.displayName || targetMember.name} ${system.systemTag}`
        : targetMember.displayName || targetMember.name;
      ...
      try {
        ...
        if (targetMember.avatarUrl) await intent.setAvatarUrl(targetMember.avatarUrl);
      } catch {
        // Ignore errors
      }
...
export const startMatrixBot = async () => {
  ...
  try {
    await botIntent.botSdkIntent.underlyingClient.doRequest('POST', '/_matrix/client/v3/register', null, {
  ...
  app.use(async (req: { method: string; path: string; body: unknown }, res: unknown, next: () => void) => {
    if (req.method === 'PUT' && req.path.includes('/transactions/')) {
      try {
        await router.processTransaction(req.body as never);
      } catch (e: unknown) {
...
const joinPendingInvites = async (bridgeInstance: Bridge) => {
  try {
    ...
    })) as { rooms?: { invite?: Record<string, unknown> } };
