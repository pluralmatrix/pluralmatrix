...
export const checkMessage = async (req: Request, res: Response) => {
  try {
    ...
    console.log(
      `[Gatekeeper] Analyzing event ${event_id} - has m.new_content: ${!!content['m.new_content']}, rel_type: ${content['m.relates_to']?.rel_type}`,
    );
    ...
    if (proxyMatch) {
      ...
      if (!isEncryptedSource) {
        console.log(
          `[Gatekeeper] Triggering background proxy for unencrypted ${event_id} for member ${proxyMatch.targetMember.slug}`,
        );
        ...
        (async () => {
          try {
              ...
              try {
                ...
                if (botClient) {
                  originalEvent = await fetchAndDecryptHistoricalEvent(
                    ...
                    originalEventId,
                    cryptoManager,
                    bot_id || botBridge?.getBot().getUserId() || sender,
                  );
            ...
            console.log(
              `[Gatekeeper] Final relatesTo sent to queue:`,
              relatesTo ? JSON.stringify(relatesTo) : 'undefined',
            );
        ...
        console.log(
          `[Gatekeeper] E2EE Match for ${event_id} - Visibility BLOCKED, but letting bot.ts handle proxying.`,
        );
      }
