...
export const uploadMedia = async (req: Request, res: Response) => {
  try {
    ...
    const response = await fetch(
      `${config.synapseUrl}/_matrix/media/v3/upload?filename=${encodeURIComponent(filename)}`,
      {
        method: 'POST',
        headers: {
...
export const downloadMedia = async (req: Request, res: Response) => {
  try {
    ...
    const response = await fetch(
      `${config.synapseUrl}/_matrix/client/v1/media/download/${server as string}/${mediaId as string}`,
      {
        headers: {
          Authorization: `Bearer ${config.asToken}`,
        },
