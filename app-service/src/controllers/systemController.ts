    ...
    while (attempts < 5) {
      try {
        ...
        if (e.code === 'P2002' && e.meta?.target?.includes('slug')) {
          attempts++;
          console.warn(
            `[SystemController] Slug race condition detected for ${localpart}, retrying (attempt ${attempts})...`,
          );
          continue;
        }
        throw err;
...
export const updateSystem = async (req: AuthRequest, res: Response) => {
  try {
    ...
    const updateData: Prisma.SystemUncheckedUpdateInput = {
      name,
      systemTag,
      autoproxyId,
      autoproxyMode,
...
export const deleteLink = async (req: AuthRequest, res: Response) => {
  try {
    ...
    if (remainingLinks.length === 0) {
      ...
      const nextPrimary =
        remainingLinks.find((l) => l.matrixId.toLowerCase() === mxid.toLowerCase()) || remainingLinks[0];
