...
describe('CrossSigningBootstrapper', () => {
  ...
  it('should execute sidecar, format payloads correctly, and proxy to Synapse', async () => {
    ...
    const keysCallArgs = (
      fetchMock.mock.calls as { method: string; headers: Record<string, string>; body: string }[][]
    )[0][1];
  ...
  it('should throw an error if the Matrix API rejects the upload', async () => {
    (fs.existsSync as jest.Mock).mockReturnValue(false);
    _mockExecFileAsync.mockResolvedValue({
      stdout: JSON.stringify({ upload_keys: {}, upload_signatures: {} }),
      stderr: '',
    });
