...
describe('Crypto Resilience Utilities', () => {
  ...
  describe('doAsRequest', () => {
    it('should retry on 429 rate limits and eventually succeed', async () => {
      ...
      const result = await doAsRequest<{ success: boolean }>(hsUrl, asToken, userId, 'POST', '/test', { foo: 'bar' });
    ...
    it('should eventually fail after max attempts', async () => {
      ...
      await expect(doAsRequest(hsUrl, asToken, userId, 'POST', '/test', {})).rejects.toThrow(/Max attempts reached/);
