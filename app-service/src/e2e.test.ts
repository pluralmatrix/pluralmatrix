...
type TestMatrixEvent = {
  ...
  content: {
    ...
    msgtype?: string;
    url?: string;
    info?: { h?: number; [key: string]: unknown };
...
describe('PluralMatrix E2E Roundtrip', () => {
   ...
   */
  async function waitForBotToJoin(
    targetClient: MatrixClient,
    targetRoomId: string,
    botUserId: string = '@plural_bot:localhost',
  ...
  async function verifyRedaction(targetClient: MatrixClient, targetRoomId: string, eventId: string, label: string) {
    ...
    for (let i = 0; i < 15; i++) {
      ...
      try {
        ...
        const clientWithCrypto = targetClient as unknown as {
          crypto?: { processRoomEvents: (roomId: string) => Promise<void> };
        };
