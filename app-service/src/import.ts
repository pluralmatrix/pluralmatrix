...
export const extractNameFromDescription = (description: string | null): string | null => {
  ...
  const patterns = [/(?:My\s+name\s+is|my\s+name\s+is|I'm|i'm|I\s+am|i\s+am)\s+([A-Z][^.!?\n,]+)/];
...
export const validateImageBuffer = (buffer: Buffer): { valid: boolean; error?: string } => {
  ...
  try {
    ...
    if (smallestAxis >= 1000) {
      return {
        valid: false,
        error: `Resolution too high (${dimensions.width}x${dimensions.height}). Smallest axis must be below 1000px.`,
      };
    }
    ...
    if (largestAxis > 4000) {
      return {
        valid: false,
        error: `Resolution too high (${dimensions.width}x${dimensions.height}). Largest axis must be 4000px or fewer.`,
      };
    }
  } catch {
 ...
 */
export const importFromPluralKit = async (
  mxid: string,
  jsonData: PKExport,
): Promise<{ count: number; systemSlug: string; failedAvatars: AvatarMigrationError[] }> => {
  console.log(`[Importer] Starting import for ${maskMxid(mxid)}`);

  const isPluralMatrix =
    jsonData.pluralmatrix_metadata !== undefined || jsonData.config?.pluralmatrix_version !== undefined;
  ...
  if (link) {
    ...
    while (attempts < 5) {
      try {
        const baseSlug =
          isPluralMatrix && (jsonData.id as string)
        ...
        if (
          ...
          (err as { code?: string; meta?: { target?: string[] } }).meta?.target?.includes('slug')
        ) {
          attempts++;
  ...
  for (const member of rawMembers) {
    let baseSlug = isPluralMatrix && member.id ? member.id : generateSlug(member.name as string, '');
  ...
  console.log(
    `[Importer] Successfully imported ${importedCount} members and ${importedGroupsCount} groups for ${maskMxid(mxid)}`,
  );
...
export const generatePkJson = async (mxid: string, avatarUrlMap?: Record<string, string>) => {
  ...
  const pkExport = {
    ...
    members: system.members.map((m) => ({
      ...
      avatar_url:
        avatarUrlMap && avatarUrlMap[m.id]
          ? avatarUrlMap[m.id]
          : m.avatarUrl?.startsWith('mxc://')
            ? null
            : m.avatarUrl || null,
    ...
    groups:
      (system as LooseSystem).groups?.map((g: LooseGroup) => ({
 ...
 */
export const importSystemZip = async (
  mxid: string,
  zipBuffer: Buffer,
): Promise<{ count: number; systemSlug: string; failedAvatars: AvatarMigrationError[] }> => {
  ...
  const jsonEntry = entries.find(
    (e) =>
      e.entryName === 'pluralmatrix_backup.json' ||
      e.entryName === 'pluralkit_system.json' ||
 ...
 */
export const importAvatarsZip = async (
  mxid: string,
  zipBuffer: Buffer,
): Promise<{ count: number; failedAvatars: AvatarMigrationError[] }> => {
  const link = await prisma.accountLink.findUnique({
    where: { matrixId: mxid },
    include: {
      system: {
  ...
  for (const entry of entries) {
    ...
    if (isGroupIcon) {
      affectedGroups = ((system as LooseSystem).groups || []).filter(
        (g: LooseGroup) => g.icon && g.icon.endsWith(`/${oldMediaId}`),
      );
    } else {
      affectedMembers = system.members.filter((m) => m.avatarUrl && m.avatarUrl.endsWith(`/${oldMediaId}`));
    }
