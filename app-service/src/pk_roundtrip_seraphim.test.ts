...
describe('Seraphim PK Roundtrip', () => {
    ...
    it('should perfectly roundtrip the Seraphim dump (excluding groups, privacy and images)', async () => {
        ...
        (cleanOriginal.members as { id: string }[]).sort((a: { id: string }, b: { id: string }) => a.id.localeCompare(b.id));
        (cleanExported.members as { id: string }[]).sort((a: { id: string }, b: { id: string }) => a.id.localeCompare(b.id));
