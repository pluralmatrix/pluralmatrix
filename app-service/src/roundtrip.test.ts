...
describe('PluralKit Roundtrip', () => {
  ...
  describe('Avatar ZIP Roundtrip', () => {
    it('should export avatars to a ZIP stream with correct data and README', async () => {
        ...
        members: [
          {
            ...
            name: 'Alice',
            slug: 'alice',
            avatarUrl: 'mxc://localhost/media1',
            createdAt: new Date(),
            proxyTags: [],
          },
        ],
      ...
      const jsonPk = JSON.parse(zipPk.getEntry('pluralkit_system.json')!.getData().toString()) as {
    ...
    it('should import avatars from a ZIP and re-upload exact binary data', async () => {
      ...
      const mockSystem = {
        id: 'sys1',
        slug: 'mysys',
        members: [{ id: 'm1', slug: 'alice', name: 'Alice', avatarUrl: 'mxc://old/media1' }],
      };
      (prisma.accountLink.findUnique as jest.Mock).mockResolvedValue({ system: mockSystem });
      ...
      expect(mockBotClient.uploadContent).toHaveBeenCalledWith(originalData, 'image/png', 'avatars/alice_media1.png');
