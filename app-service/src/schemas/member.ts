...
export const MemberPrivacySchema = z
  .object({
...
export const SystemPrivacySchema = z
  .object({
    description_privacy: PrivacyLevelSchema.default('public'),
...
export const MemberSchema = z.object({
  ...
  avatarUrl: z.preprocess(
    ...
    z
      .string()
      .max(256, 'Avatar URL must be 256 characters or fewer')
      .url()
      .or(z.string().startsWith('mxc://'))
      .optional()
      .nullable(),
  ),
  ...
  slug: z
    .string()
    .regex(/^[a-z0-9-]+$/, 'Short ID must be alphanumeric with hyphens')
    .max(50),
...
export const SystemSchema = z.object({
  ...
  avatarUrl: z.preprocess(
    emptyToNull,
    z.string().max(256).url().or(z.string().startsWith('mxc://')).optional().nullable(),
  ),
