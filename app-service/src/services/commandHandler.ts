...
export class CommandHandler {
    ...
    try {
      ...
      if (err.errcode === 'M_FORBIDDEN' || err.httpStatus === 403) {
        try {
          ...
          if (
            (fbErr.errcode === 'M_FORBIDDEN' || fbErr.httpStatus === 403) &&
            !this.permissionWarnedRooms.has(roomId)
          ) {
  ...
  async getRoomMessages(roomId: string, limit: number = 50): Promise<{ chunk: PluralMatrixEvent[] }> {
    return this.bridge
      .getBot()
      .getClient()
      .doRequest('GET', `/_matrix/client/v3/rooms/${encodeURIComponent(roomId)}/messages`, {
        limit,
        dir: 'b',
  ...
  async resolveGhostMessage(roomId: string, systemSlug: string | undefined, explicitTargetId?: string) {
    ...
    if (rootId) {
      ...
      try {
        ...
        explicitEvent =
          scrollback.chunk.find((e: PluralMatrixEvent) => e.event_id === rootId || e.id === rootId) || null;
        if (!explicitEvent) {
  ...
  async executeTargetingCommand(event: PluralMatrixEvent, body: string, system: SystemWithRelations | null) {
      ...
      if (subCmd === '-delete' || subCmd === '-d') {
        if (system?.slug && targetSender.startsWith(`@_plural_${system.slug}_`)) {
          ...
          await this.sendEncryptedText(
            this.bridge.getIntent(),
            roomId,
            'You can only delete your own proxied messages.',
          );
        }
      } else {
    ...
    await registerDevice(
      this.bridge.getIntent(targetSender),
      (await this.cryptoManager.getMachine(targetSender)).deviceId.toString(),
      this.prisma,
      memberId,
      systemId,
    );
    ...
    if (cmd === 'edit' || cmd === 'e') {
      ...
      const editResultId = await sendEncryptedEvent(
        this.bridge.getIntent(targetSender),
        roomId,
        'm.room.message',
        editPayload,
      ...
      if (member) {
        if (!latestText) {
          await this.sendEncryptedText(
            this.bridge.getIntent(),
            roomId,
            "Could not extract the message text to reproxy. This usually happens if the bot can't decrypt the original message.",
          );
          return true;
        }
  ...
  async handleMessageInfoRequest(
    roomId: string,
    requestingUserId: string,
    targetEventId: string,
    replyInRoom: boolean = false,
  ) {
    try {
      const botClient = (this.bridge.getIntent() as IntentWithClient).matrixClient;
      ...
      if (!targetEvent || !targetEvent.sender.startsWith('@_plural_')) {
        if (replyInRoom) {
          await this.sendEncryptedText(
            this.bridge.getIntent(),
            roomId,
            'That message does not appear to be a proxied message.',
          );
        }
        return;
      }
      ...
      const formattedBody =
        `<strong>Message Information</strong><br>` +
        `<ul>` +
...
**Data Control:** We don't use tokens. Use the web UI to export your data regularly for backups or to move servers.`;
          ...
          await this.sendRichText(
            this.bridge.getIntent(),
            roomId,
            '❌ Failed to create system due to an internal error.',
          );
          return true;
        }
      }
      ...
      if (!subCmd) {
        ...
        if (isOwnSystem || sysPrivacy.member_list_privacy !== 'private') {
          const visibleMembers =
            targetSystem.members?.filter((m) => isOwnSystem || getMemberPrivacy(m.privacy).visibility !== 'private') ||
            [];
      ...
      if (subCmd === 'list') {
        ...
        if (visibleMembers.length === 0) {
          await this.sendEncryptedText(
            this.bridge.getIntent(),
            roomId,
            'No system members found or they are all private.',
          );
          return true;
        }
        ...
        const sysName =
          targetSystem.name && (isOwnSystem || sysPrivacy.name_privacy !== 'private') ? targetSystem.name : 'System';
      ...
      if (subCmd === 'description' || subCmd === 'desc') {
        ...
        await this.sendRichText(
          this.bridge.getIntent(),
          roomId,
          newDesc ? `✅ System description updated.` : `✅ System description cleared.`,
        );
        return true;
      }
      ...
      if (subCmd === 'tag') {
        ...
        await this.sendRichText(
          this.bridge.getIntent(),
          roomId,
          newTag ? `✅ System tag updated to \`${newTag}\`.` : `✅ System tag cleared.`,
        );
        return true;
      }
      ...
      if (subCmd === 'avatar' || subCmd === 'icon') {
        ...
        await this.sendEncryptedText(
          this.bridge.getIntent(),
          roomId,
          'Usage: `pk;system avatar <url>` or attach an image.',
        );
        return true;
      }
      ...
      if (subCmd === 'delete') {
        ...
        await this.sendRichText(
          this.bridge.getIntent(),
          roomId,
          `⚠️ To delete your system, please use the Web Dashboard Settings: ${buildWebUrl()}`,
        );
        return true;
      }
    }
    ...
    if (cmd === 'link') {
      if (!parts[1]) {
        await this.sendEncryptedText(
          this.bridge.getIntent(),
          roomId,
          'Usage: `pk;link <@user:domain>` or `pk;link primary <@user:domain>`',
        );
        return true;
      }
      ...
      try {
        ...
        await this.sendRichText(
          this.bridge.getIntent(),
          roomId,
          `❌ Could not verify Matrix ID **${targetMxid}**. Please ensure the ID is correct and the user exists.`,
        );
        return true;
      }
      ...
      if (parts[1].toLowerCase() === 'primary' && parts[2]) {
        ...
        if (!link || link.systemId !== system.id) {
          await this.sendRichText(
            this.bridge.getIntent(),
            roomId,
            `**${targetMxid}** is not linked to your system. Link it first with \`pk;link ${targetMxid}\`.`,
          );
          return true;
        }
        ...
        await this.sendRichText(
          this.bridge.getIntent(),
          roomId,
          `✅ **${targetMxid}** is now the primary routing account for this system. Direct messages sent to system members will be forwarded here.`,
        );
        return true;
      }
      ...
      if (!system) {
        await this.sendRichText(
          this.bridge.getIntent(),
          roomId,
          'You must have a system to link other accounts to it.',
        );
        return true;
      }
      ...
      if (targetLink) {
        if (targetLink.systemId === system.id) {
          await this.sendRichText(
            this.bridge.getIntent(),
            roomId,
            `**${targetMxid}** is already linked to your system.`,
          );
          return true;
        }
        ...
        if (targetLink.system.members.length > 0) {
          await this.sendRichText(
            this.bridge.getIntent(),
            roomId,
            `❌ **${targetMxid}** already belongs to an active system with members. They must delete their system before they can be linked to yours.`,
          );
          return true;
        }
      ...
      await this.sendRichText(
        this.bridge.getIntent(),
        roomId,
        `✅ Successfully linked **${targetMxid}** to this system (\`${system.slug}\`).`,
      );
      return true;
    }
    ...
    if (cmd === 'unlink') {
      ...
      if (targetMxid === sender) {
        await this.sendRichText(
          this.bridge.getIntent(),
          roomId,
          'You cannot unlink your own primary account from the system.',
        );
        return true;
      }
      ...
      if (remainingLinks <= 1) {
        // 1 because we are about to delete it, so if it's 1 right now, it's the last one
        await this.sendRichText(
          this.bridge.getIntent(),
          roomId,
          'You cannot unlink the last account from a system. Use the Web UI to delete the entire system instead.',
        );
        return true;
      }
      ...
      await this.sendRichText(
        this.bridge.getIntent(),
        roomId,
        `✅ Successfully unlinked **${targetMxid}** from this system.`,
      );
      return true;
    }
    ...
    if (cmd === 'autoproxy' || cmd === 'auto' || cmd === 'ap') {
      ...
      if (targetSlug === 'latch') {
        ...
        await this.sendEncryptedText(
          this.bridge.getIntent(),
          roomId,
          'Autoproxy latch mode enabled. The last proxied member will be automatically locked in.',
        );
        return true;
      }
    ...
    if (cmd === 'group' || cmd === 'g') {
      ...
      if (!subCmd || subCmd === 'list') {
        ...
        const groupList = sortedGroups
          .map((g) => `* **${g.displayName || g.name}** (id: \`${g.slug}\`) - ${g.members?.length || 0} members`)
          .join('\n');
        await this.sendRichText(
          this.bridge.getIntent(),
          roomId,
          `### ${system.name || 'Your System'} Groups\n${groupList}`,
        );
        return true;
      }
      ...
      if (action === 'add' || action === 'remove') {
        ...
        if (memberSlugs.length === 0) {
          await this.sendEncryptedText(
            this.bridge.getIntent(),
            roomId,
            `Usage: \`pk;group ${groupSlug} ${action} <member1> <member2> ...\``,
          );
          return true;
        }

        const validMembers = system.members.filter(
          (m) => memberSlugs.includes(m.slug) || (m.pkId && memberSlugs.includes(m.pkId)),
        );
        ...
        if (action === 'add') {
          ...
          await this.sendRichText(
            this.bridge.getIntent(),
            roomId,
            `✅ Added ${validMembers.length} member(s) to **${group.name}**.`,
          );
        } else {
          await this.prisma.group.update({
            where: { id: group.id },
          ...
          await this.sendRichText(
            this.bridge.getIntent(),
            roomId,
            `✅ Removed ${validMembers.length} member(s) from **${group.name}**.`,
          );
        }
      ...
      if (action === 'rename') {
        ...
        if (!newName) {
          await this.sendEncryptedText(
            this.bridge.getIntent(),
            roomId,
            `Usage: \`pk;group ${groupSlug} rename <new name>\``,
          );
          return true;
        }
        await this.prisma.group.update({
      ...
      if (action === 'description' || action === 'desc') {
        ...
        await this.sendRichText(
          this.bridge.getIntent(),
          roomId,
          newDesc ? `✅ Updated group description.` : `✅ Cleared group description.`,
        );
        return true;
      }
      ...
      if (action === 'icon') {
        ...
        await this.sendRichText(
          this.bridge.getIntent(),
          roomId,
          newIcon ? `✅ Updated group icon.` : `✅ Cleared group icon.`,
        );
        return true;
      }
  ...
  async promoteSystemPowerLevels(roomId: string, ghostUserId: string) {
    try {
      ...
      const state = (await (ghostIntent as IntentWithClient).matrixClient.getRoomStateEvent(
        roomId,
        'm.room.power_levels',
        '',
      ...
      if (changed) {
        ...
        await (this.bridge.getIntent(ghostUserId) as IntentWithClient).matrixClient.sendStateEvent(
          roomId,
          'm.room.power_levels',
          '',
          state,
