    ...
    try {
      ...
      while (queue.length > 0) {
        ...
        try {
          ...
          const payload: PluralMatrixEventContent = item.fullContent
            ? { ...item.fullContent }
            : { msgtype: 'm.text', body: item.plaintext };
          ...
          if (item.systemSlug && result?.event_id) {
            ...
            const rootId = isReplacement
              ? item.relatesTo!.event_id || (item.relatesTo as { id?: string }).id
              : result.event_id;
  ...
  private async handleSendError(item: QueueItem, queue: QueueItem[], error: unknown) {
    ...
    if (!isFatal) {
      ...
      if (item.attempts <= 3) {
        ...
        console.warn(
          `[Queue] Transient error sending for ${item.ghostIntent.userId} in ${item.roomId}. Retrying in ${Math.round(waitTime)}ms...`,
    ...
    console.error(
      `[Queue] Delivery failed for ${item.ghostIntent.userId}. Triggering Fallback 1 (Bot Bailout). Error:`,
      (error as Error).message,
    );
    ...
    try {
      ...
      console.error(
        `[Queue] Fallback 1 (Bot Bailout) failed! Triggering Fallback 2 (Dead Letter Vault).`,
