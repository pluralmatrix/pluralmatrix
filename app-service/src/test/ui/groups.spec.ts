...
test.describe('Web UI Groups Flow', () => {
  ...
  test('User can create, edit, and delete groups, and add members', async ({ page }) => {
    ...
    const memberAvatarUploadPromise = page.waitForResponse(
      (response) => response.url().includes('/api/media/upload') && response.status() === 200,
    );
    await page
      .locator('[data-testid="avatar-upload-input"]')
      .setInputFiles(path.resolve(__dirname, 'fixtures/dummy.png'));
    ...
    // Upload icon
    const groupIconUploadPromise = page.waitForResponse(
      (response) => response.url().includes('/api/media/upload') && response.status() === 200,
    );
    await page
      .locator('[data-testid="icon-upload-input"]')
      .setInputFiles(path.resolve(__dirname, 'fixtures/dummy.png'));
