...
test.describe('Web UI Groups Flow', () => {
    ...
    test('User can create, edit, and delete groups, and add members', async ({ page }) => {
        ...
        // Select Lily
        await page.click('button:has-text("Lily")');
        ...
        await expect(page.locator('span:has-text("Cool Group")')).toBeVisible();
        ...
        await expect(page.locator('button:has-text("Cool Group")')).toHaveClass(/bg-matrix-primary/);

        // Click "Another Group" to select it
        await page.click('button:has-text("Another Group")');
        await page.getByTestId('save-member-button').click();

        // Verify member now has both group tags
        await expect(page.locator('span:has-text("Cool Group")')).toBeVisible();
        await expect(page.locator('span:has-text("Another Group")')).toBeVisible();

        // Step 5: Edit Group
        await page.click('button:has-text("Groups (2)")');
        await page.hover('h3:has-text("Cool Group")');
        await page.locator('button[title="Edit Group"]').click();
