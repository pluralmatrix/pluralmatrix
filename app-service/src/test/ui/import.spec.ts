...
test.describe('Web UI System Import and Data Flow', () => {
  ...
  test('User can import a system via JSON, manage settings, and view members', async ({ page }) => {
    ...
    const importPromise = page.waitForResponse(
      (response) => response.url().includes('/api/import/pk/json') && response.request().method() === 'POST',
    );
    ...
    const updateMemberPromise = page.waitForResponse(
      (response) =>
    ...
    const autoproxyEnablePromise = page.waitForResponse(
      (response) =>
        response.url().includes('/api/system') && response.request().method() === 'PATCH' && response.status() === 200,
    );
    ...
    const autoproxyDisablePromise = page.waitForResponse(
      (response) =>
        response.url().includes('/api/system') && response.request().method() === 'PATCH' && response.status() === 200,
    );
    ...
    const deleteMemberPromise = page.waitForResponse(
      (response) =>
        response.url().includes('/api/members/') &&
  ...
  test('User can import a full backup via ZIP including avatars', async ({ page }) => {
    ...
    const importPromise = page.waitForResponse(
      (response) => response.url().includes('/api/import/backup/zip') && response.request().method() === 'POST',
    );
