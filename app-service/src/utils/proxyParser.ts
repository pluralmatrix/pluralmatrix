...
export function parseProxyMatch(
  content: PluralMatrixEventContent,
  system: SystemWithRelations,
  originalEventContent?: PluralMatrixEventContent,
  ...
  if (rawFormattedBody) {
    if (matchedPrefixLength > 0 && rawFormattedBody.startsWith(rawBody.substring(0, matchedPrefixLength))) {
      const stripped = rawFormattedBody
        .slice(matchedPrefixLength, rawFormattedBody.length - matchedSuffixLength)
        .trim();
