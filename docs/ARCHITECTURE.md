...
4. **The Blackhole:** Synapse receives `BLOCK`. Using the `check_visibility_can_see_event` hook, it hides the original message from everyone _except_ the original sender and the bot. To onlookers, the original message never existed.
...
- **Decryption Retries:** Decryption keys (Megolm sessions) often arrive _after_ the encrypted message in federated or highly active rooms. `gatekeeperController.ts` implements a brief retry loop (`await new Promise(resolve => setTimeout(resolve, 200));`) to wait for keys if the first decryption attempt fails.
- **Edit Handling:** Edits (`m.relates_to` -> `m.replace`) are handled natively. PluralMatrix redacts the _original_ root message when an edit is proxied, as Matrix servers automatically cascade redactions to all associated `m.replace` events.
