...
_(Note: The "Zero-Flash" feature requires our custom Synapse module. If you use an external non-Synapse server, proxying will work normally, but original proxy trigger messages may briefly "flash" in the timeline before being deleted and replaced by the bot.)_
...
4. **Important:** Copy the `app-service-registration.yaml` file to your _actual_ homeserver and configure it to load the file. Keep the `.env` file handy.
...
_For complex multi-container stateful deployments like this, we still strongly recommend using a standard VPS with our provided `docker-compose.yml` for the smoothest experience._
